...
def train_fastAA(dataset_name, K, N, T, B, epochs, batch_size, lr=0.001, weight_decay=0.0001, patience=100):
    ...
    for fold in range(K):
        ...
        T_star.append(bayesian_optimization(model, train_folds[fold], N, T, B, criterion, num_opt=2))
    ...
    augmented_train_loader,_,_ = getDataLoader(dataset_name, batch_size,transform=T_star,num_opt=2)
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if val_log['acc'] > best_acc:
            best_acc = val_log['acc']
            torch.save(model_augmented.state_dict(), 'best_model_augmented.pth')
